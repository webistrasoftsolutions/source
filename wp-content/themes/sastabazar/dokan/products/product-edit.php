...
if ( class_exists( 'Dokan_Product_Subscription' ) ) {
    if ( get_user_meta( $seller_id, 'product_no_with_pack', true ) ) {
        if ( get_user_meta( $seller_id, 'product_no_with_pack', true ) <= count( get_posts( array( 'post_author' => $seller_id, 'post_type' => 'product', 'posts_per_page' => -1 ) ) ) || 'publish' != $post->post_status ) {
            wp_die( esc_html__( 'Access Denied! You have reached the limit of your subscribed total online product', 'sanzo' ) );
        }
    }
}
        ...
        <div class="dokan-product-edit-area">
            ...
            <header class="dokan-pro-edit-breadcrumb">
                <h1 class="dokan-header-crumb">
                    <span class="dokan-breadcrumb"><a href="<?php echo dokan_get_navigation_url( 'products' ) ?>"><?php esc_html_e( 'Products', 'sanzo' ); ?></a> &rarr; </span>
                    <?php echo $post->post_title; ?>

                    <?php if ( $_visibility == 'hidden' ) { ?>
                        <span class="dokan-label dokan-label-default"><?php esc_html_e( 'Hidden', 'sanzo' ); ?></span>
                    <?php } ?>

                    <?php if ( $post->post_status == 'publish' ) { ?>
                        <span class="dokan-right">
                            <a class="view-product dokan-btn dokan-btn-sm" href="<?php echo esc_url( get_permalink( $post->ID ) ); ?>" target="_blank"><?php esc_html_e( 'View Product', 'sanzo' ); ?></a>
                        </span>
                    <?php } ?>
                </h1>
            </header>
                        ...
                        <div class="dokan-product-edit-left">
                            ...
                            <?php if ( isset( $_GET['message'] ) && $_GET['message'] == 'success') { ?>
                                <div class="dokan-message">
                                    ...
                                    <strong><?php esc_html_e( 'Success!', 'sanzo' ); ?></strong> <?php esc_html_e( 'The product has been updated successfully.', 'sanzo' ); ?>

                                    <?php if ( $post->post_status == 'publish' ) { ?>
                                        <a href="<?php echo esc_url( get_permalink( $post_id ) ); ?>" target="_blank"><?php esc_html_e( 'View Product &rarr;', 'sanzo' ); ?></a>
                                    <?php } ?>
                                </div>
                            <?php } ?>
                            ...
                            <div id="tab-container" class='tab-container'> <!-- Only required for left/right tabs -->
                                ...
                                <ul class="dokan_tabs">
                                    ...
                                    $dokan_product_data_tabs = apply_filters( 'dokan_product_data_tabs', array(
                                        ...
                                        'edit' => array(
                                            'label'  => esc_html__( 'Edit', 'sanzo' ),
                                            'target' => 'edit-product',
                                            'class'  => array( 'active' ),
                                        ),
                                        'options' => array(
                                            'label'  => esc_html__( 'Options', 'sanzo' ),
                                            'target' => 'product-options',
                                            'class'  => array(),
                                        ),
                                        'inventory' => array(
                                            'label'  => esc_html__( 'Inventory', 'sanzo' ),
                                            'target' => 'product-inventory',
                                            'class'  => array(),
                                        ),
                                        'shipping' => array(
                                            'label'  => esc_html__( 'Shipping', 'sanzo' ),
                                            'target' => 'product-shipping',
                                            'class'  => array( 'hide_if_virtual' ),
                                        ),
                                        'attributes' => array(
                                            'label'  => esc_html__( 'Attributes', 'sanzo' ),
                                            'target' => 'product-attributes',
                                            'class'  => array(),
                                        ),
                                        'variations' => array(
                                            'label'  => esc_html__( 'Variations', 'sanzo' ),
                                ...
                                <div id="tabs_container">
                                    <div id="edit-product">
                                                ...
                                                <div class="show_if_simple">
                                                    <div class="special-price-container">
                                                        ...
                                                        <div class="dokan-form-group dokan-clearfix">
                                                            <div class="dokan-input-group">
                                                                <span class="dokan-input-group-addon"><?php echo get_woocommerce_currency_symbol(); ?></span>
                                                                <?php dokan_post_input_box( $post_id, '_sale_price', array( 'placeholder' => esc_html__( 'Special Price', 'sanzo' ) ) ); ?>
                                                            </div>
                                                            <a href="#" class="sale-schedule dokan-right"><?php esc_html_e( 'Schedule', 'sanzo' ); ?></a>
                                                        </div>
                                        ...
                                        <div class="dokan-rich-text-wrap">
                                            <div class="dokan-form-group">
                                                <?php dokan_post_input_box( $post_id, 'post_excerpt', array( 'placeholder' => esc_html__('Short description about the product...', 'sanzo'), 'value' => $post->post_excerpt ), 'textarea' ); ?>
                                            </div>
